...
@section('content')
    ...
    <div class="w-full max-w-xl bg-zinc-300 p-8 rounded-lg shadow-lg shadow-indigo-500/50">
        ...
        <h1 class="text-2xl font-bold text-center mb-6">Criar Nova Solicitação</h1>
        ...
        <form action="{{ route('solicitations.store') }}" method="POST" class="space-y-4 ">
            @csrf
            ...
            <div>
                ...
                <select name="type_" class="w-full px-4 py-2 rounded-[10vw] bg-white border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="{{ old('type_')}}">Selecione...</option>
                    <option value="feadback" @selected(old('type_')=='feadback')>feadback</option>
                    <option value="new_obra" @selected(old('type_')=='new_obra')>Nova Obra</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="ask_" placeholder="Descreva Sua Solicitação" value="{{ old('ask_') }}"class="w-full px-4 py-2 rounded-[10vw] bg-white border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
            </div>
