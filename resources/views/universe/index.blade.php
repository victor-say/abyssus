    ...
    <main class="flex-1 p-6">
        ...
        <header class="w-full bg-zinc-300 shadow-lg shadow-indigo-500/50 rounded-lg p-4 mb-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-semibold">Universos</h1>
                </div>
            ...
            <div class="mt-4">
                <a href="{{ route('universes.create')}}" class="inline-flex items-center gap-2 px-4 py-2 bg-cyan-500 text-white font-medium rounded hover:bg-cyan-600 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                ...
                <table class="w-full">
                    <thead class="bg-slate-700 text-white">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-sm font-semibold">Personagens</th>
                            <th class="px-6 py-3 text-left text-sm font-semibold">Livros</th>
                            <th class="px-6 py-3 text-left text-sm font-semibold">Conceitos</th>
                    ...
                    <tbody class="divide-y divide-gray-400">
                        @forelse ($universes as $universe)
                            <tr class="hover:bg-gray-200 transition-colors">
                                <td class="px-6 py-3">{{ $universe->name }}</td>
                                <td class="px-6 py-3">{{ $universe->author }}</td>
                                <td class="px-6 py-3">{{ $universe->personagens }}</td>
                                <td class="px-6 py-3">{{ $universe->books }}</td>
                                <td class="px-6 py-3">{{ $universe->conceitos }}</td>
                                ...
                                <td class="px-6 py-3 text-center">
                                    <a href="{{ route('universes.show', $universe->id) }}" class="inline-flex items-center gap-1 px-3 py-1 bg-blue-800 text-white text-sm rounded hover:bg-blue-600 transition-colors hover:-translate-y-1">
                                        Detalhes
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-600">
                                    Nenhum Universo encontrado no banco
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
